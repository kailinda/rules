...
if [ "x$raw_fastgit_net_code" = "x1" ] ; then
	...
	{ logger -t "【ADBLOCK】" "更新bogus-nxdomain.conf,失败 !!!";ret_r=1;}
	rm -f /tmp/bogus-nxdomain.conf.tmp
else
...
if [ "x$raw_fastgit_net_code" = "x1" ] ; then
	...
	{ logger -t "【ADBLOCK】" "更新 github加速文件,失败 !!!";ret_r=1;}
	rm -f /tmp/fast-github.host.tmp
else
