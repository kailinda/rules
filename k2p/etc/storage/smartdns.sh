...
if [[ "x$net_code1" = "x1" -o "x$net_code2" = "x1" ]]; then
	...
	{ logger -t "${logger_title}" "【SMART_DNS】smartdns.tar.bz2合成 失败 !!!";exit 0;}
	...
	{ logger -t "${logger_title}" "【SMART_DNS】smartdns.tar.bz2解压 失败 !!!";exit 0;}
	...
	/sbin/start-stop-daemon -S -c nobody -b -o -q -x /tmp/bin/smartdns -- -c /tmp/conf/smart.conf -p /tmp/smart.pid && logger -t "${logger_title}" "【SMART_DNS】smartdns启动,成功 ~~~" || { logger -t "${logger_title}" "【SMART_DNS】smartdns启动,失败 !!!";exit 0;}
	/tmp/bin/pdnsd -d -4 -c /tmp/conf/pdnsd.conf && logger -t "${logger_title}" "【SMART_DNS】pdnsd启动,成功 ~~~" || { logger -t "${logger_title}" "【SMART_DNS】pdnsd启动,失败 !!!";exit 0;}
